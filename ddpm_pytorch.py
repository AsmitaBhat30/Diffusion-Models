...
class SimpleUnet(nn.Module):
    ...
    def __init__(self):
        ...
        # Downsample
        self.downsampling_nw = nn.ModuleList(
            [Block(down_channels[i], down_channels[i + 1], time_embedding_dim) for i in range(len(down_channels) - 1)])

        self.upsampling_nw = nn.ModuleList(
            [Block(up_channels[i], up_channels[i + 1], time_embedding_dim, up=True) for i in
             range(len(up_channels) - 1)])
...
Path("./results/loss").mkdir(parents=True, exist_ok=True)

with open("./results/loss/loss.txt", "w+") as f:
    ...
    for epoch in range(epochs):
        for step, batch in enumerate(dataloader):
            ...
            if epoch % 5 == 0 and step == 0:
                ...
                f.write(f'Epoch {epoch} | step {step:03d} Loss : {loss.item()} \n')
                sample_plot_image('./results/imgs/Epoch_' + str(epoch))
